<#
.SYNOPSIS
    Builds the pinned x86_64 Vulkan runtime (Vulkan loader and MoltenVK) that
    tools/package-macos-vulkan.ps1 bundles into osx-x64 clients.
.DESCRIPTION
    Homebrew no longer builds Intel bottles and the LunarG SDK installer runs
    only on Apple silicon, so the osx-x64 payload takes both libraries from
    Khronos directly: the MoltenVK release archive (checksum pinned) and the
    Vulkan loader built from its SDK tag (commit pinned). The output directory
    is reused while its stamp matches these pins, which lets CI cache it.

    Intel-only. Remove it with the rest of osx-x64 support; see
